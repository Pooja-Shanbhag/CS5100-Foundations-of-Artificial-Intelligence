...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles:
            newPos = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            newSample = util.sample(newPos)
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        weights = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(weights)
        self.particles = []
        flag = True
        while flag is True:
            for states in weights:
                if len(self.particles) == self.numParticles:
                    flag = False
                self.particles.append(states)
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
            if noisyDistances[0] is None:
                particle = self.getParticleWithGhostInJail(particle, 0)
                for em in emissionModels[0]:
                    emissionModels[0][em] = 1.0

            if noisyDistances[1] is None:
                particle = self.getParticleWithGhostInJail(particle, 1)
                for em in emissionModels[1]:
                    emissionModels[1][em] = 1.0
            allPossible[particle] += (emissionModels[0][util.manhattanDistance(particle[0], pacmanPosition)] * emissionModels[1][util.manhattanDistance(particle[1], pacmanPosition)])
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            for i in range(self.numGhosts):
                newParticle[i] = util.sample(getPositionDistributionForGhost(setGhostPositions(gameState, newParticle), i,
                                                           self.ghostAgents[i]))
